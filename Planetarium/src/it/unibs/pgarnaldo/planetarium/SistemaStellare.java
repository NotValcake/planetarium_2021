...
public class SistemaStellare {
	...
	public void addPianeta(Pianeta p) {
		if(this.stella.addPianeta(p)) this.addInfoCDM(p);
	}
	...
	public void addLuna(Pianeta p, Luna l) {
		...
		if(pianeta != null) {
			if(pianeta.addLuna(l)) this.addInfoCDM(l);
		}else {
			System.out.println(DIVIDER);
	...
	public int getCollisioni(){
		int collisioni = 0;
		//controllo se i pianeti della stella sono nella stessa orbita
		if(this.stella.getPianeti().size() == 1) return collisioni;
		for(int i = 0; i < this.stella.getPianeti().size(); i++){
			Pianeta p1 = this.stella.getPianeta(i);
			for(int j = 0; j < p1.getLune().size(); j++){
				Luna l = p1.getLune().get(j);
				for(int k = j + 1; k < p1.getLune().size(); k++){
					if(l.getDistanza(p1) - p1.getLune().get(k).getDistanza(p1) < EPSILON){
						collisioni++;
					}
				}
			}
	...
	public void cosaFare(int scelta) {
		...
		case Menu.NEW_PIANETA:
			...
			this.addPianeta(p);
			//InputDati.leggiStringa(String.format(LITTLE_DIV + "\n" + "Premi invio per continuare...\n"));
			break;
...
//			break;
		...
		case Menu.CATASTROFE:
			System.out.println(DIVIDER);
			boolean scelta_3 = InputDati.trueOrfalse("Vuoi eliminare una luna o un pianeta? ");
			if(scelta_3) {
				String nome_corpo = InputDati.leggiStringaNonVuota("Inserisci il nome: ");
				Luna l_2 = new Luna(nome_corpo);
				this.removeLuna(l_2);
			...
			InputDati.leggiStringa(String.format(LITTLE_DIV + CONTINUARE));
			break;
